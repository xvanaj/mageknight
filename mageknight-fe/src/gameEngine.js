...
const migrateState=state=>{
  const players=state.players||[state.player];players.filter(Boolean).forEach(player=>{player.removed=player.removed||[];player.defeated=player.defeated||[];player.tacticUsed=Boolean(player.tacticUsed);player.skipNextTurn=Boolean(player.skipNextTurn);player.roundOrderFaceDown=Boolean(player.roundOrderFaceDown);player.cardsPlayedThisTurn=player.cardsPlayedThisTurn??player.played?.length??0;player.atTurnStart=Boolean(player.atTurnStart);player.emptyHandPassAllowed=Boolean(player.emptyHandPassAllowed);player.movedThisTurn=Boolean(player.movedThisTurn);player.turnAction=player.turnAction||null;player.units=(player.units||[]).map(unit=>({...unit,wounded:Boolean(unit.wounded),woundCount:unit.woundCount||0}));});
...
export function calculateScore(state){
  ...
  const rows=players.map(player=>{
    const cards=[...player.deck,...player.hand,...player.discard,...player.played],identity=player.id||player.character,owned=state.map.filter(hex=>hex.ownerId===identity),cityScore=state.map.filter(hex=>hex.site==='city'&&hex.conquered).reduce((sum,hex)=>sum+(hex.ownerId===identity?7:(hex.conquerors||[]).includes(identity)?4:0),0);
...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'START_COMBAT': {
      ...
      const defenders=clone(hex.enemy.members||hex.enemies||[hex.enemy]).map(enemy=>({...((hex.site==='city')?withCityBonus(enemy,hex.cityColor):enemy),siteFortified:kind==='fortified'}));let enemies=defenders,enemySources=Object.fromEntries(defenders.map(enemy=>[enemyKey(enemy),{q:hex.q,r:hex.r,site:hex.site,siteDefender:true}]));
...
function resolveCooperativeAssault(state){
  ...
  const hex=state.map.find(item=>item.q===assault.q&&item.r===assault.r),enemy=enemyGroup((hex.enemy.members||hex.enemies||[hex.enemy]).map(member=>withCityBonus(member,hex.cityColor)));const pool=Object.values(assault.contributors).reduce((sum,part)=>{Object.keys(sum).forEach(key=>sum[key]+=part[key]||0);return sum;},combatContribution({}));
  ...
  const participants=state.players.filter(player=>assault.ready.includes(player.id)),power=id=>Object.values(assault.contributors[id]||{}).reduce((sum,value)=>sum+(Number(value)||0),0),ranked=[...participants].sort((a,b)=>power(b.id)-power(a.id)||participants.indexOf(a)-participants.indexOf(b)),members=enemy.members||[enemy];hex.cityShields=hex.cityShields||{};hex.cityShieldOrder=hex.cityShieldOrder||[];members.forEach((member,index)=>{const player=ranked[index%ranked.length];hex.cityShields[player.id]=(hex.cityShields[player.id]||0)+1;hex.cityShieldOrder.push(player.id);bindPlayerState(state,player.id);gainFame(state,member.fame);});const best=Math.max(...Object.values(hex.cityShields)),leaders=Object.keys(hex.cityShields).filter(id=>hex.cityShields[id]===best);hex.enemy=null;hex.enemies=[];hex.conquered=true;hex.ownerId=hex.cityShieldOrder.find(id=>leaders.includes(id))||leaders[0];hex.conquerors=Object.keys(hex.cityShields);participants.forEach(player=>{player.assaults=(player.assaults||0)+1;});const cityLeader=playerById(state,hex.ownerId);if(cityLeader)cityLeader.cities=(cityLeader.cities||0)+1;bindPlayerState(state,assault.leaderId);state.player.q=hex.q;state.player.r=hex.r;state.phase='action';state.cooperativeAssault=null;log(state,`The party conquered the ${hex.cityColor} city together; ${cityLeader?.name||'the leading conqueror'} controls it.`);checkVictory(state);return state;
}
...
function conquerFortifiedSite(state,hex){
  if(!hex||hex.conquered)return;state.player.q=hex.q;state.player.r=hex.r;hex.conquered=true;hex.ownerId=state.player.id||state.player.character;hex.enemy=null;hex.enemies=[];
...
function endTurn(state,roundAnnouncement=false) {
  ...
  state.player.discard.push(...state.player.played); state.player.played=[];state.player.cardsPlayedThisTurn=0;state.player.atTurnStart=false;state.player.emptyHandPassAllowed=false;state.player.movedThisTurn=false;state.player.turnAction=null; state.points=freshPoints();if(carry)Object.entries(carry).forEach(([key,value])=>state.points[key]+=value); state.mana=[]; state.sourceTaken=false;state.bonuses.sideways=null;state.bonuses.manaOverload=null;
...
function nextRound(state) {
  ...
  if(state.multiplayer){
    state.players.forEach((player,index)=>{bindPlayerState(state,player.id);player.deck=shuffled([...player.discard,...player.hand],state.seed+state.round+index*997);player.hand=[];player.discard=[];player.tactic=null;player.tacticUsed=false;player.cardsPlayedThisTurn=0;player.atTurnStart=false;player.emptyHandPassAllowed=false;player.movedThisTurn=false;player.turnAction=null;draw(state,handLimit(state));player.units.forEach(unit=>unit.spent=false);player.skills.forEach(skill=>{skill.used=false;skill.roundUsed=false;});});
  ...
  } else {state.player.deck=shuffled([...state.player.discard,...state.player.hand],state.seed+state.round);state.player.hand=[];state.player.discard=[];state.player.tactic=null;state.player.tacticUsed=false;state.player.cardsPlayedThisTurn=0;state.player.atTurnStart=!state.tacticsEnabled;state.player.emptyHandPassAllowed=false;state.player.movedThisTurn=false;state.player.turnAction=null;draw(state,handLimit(state));state.player.units.forEach(unit=>{unit.spent=false;});state.player.skills.forEach(s=>{s.used=false;s.roundUsed=false;});}
