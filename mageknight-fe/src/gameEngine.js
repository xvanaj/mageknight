...
export function createGame(seed = 20260901, options = {}) {
  ...
  const state = {
    ...
    points: freshPoints(), mana: [], sourceTaken: false, combat: null, pendingRewards:[], bonuses:freshBonuses(),pendingCardBoost:null,pendingBorrowedEffect:null,pendingRepeatedEffect:null,pendingSpellEffect:null,pendingManaMeltdown:null,pendingMeditation:null,pendingMindRead:null,pendingHorn:null, log: [], error: null,
  ...
  if (sidewaysAs) {
    ...
    if (resolved in state.points) state.points[resolved] += amount;
      ...
      if(amount==='basic'){
        ...
        if(choices.woundDestination==='discard'){state.player.wounds++;state.player.discard.push({id:'wound',uid:`wound-${state.turn}-blood-${state.player.wounds}`,name:'Wound',color:'wound',basic:{},strong:{}});}else wound(state,1);
...
function woundPlayer(player,state,source,count=1){player.wounds+=count;for(let index=0;index<count;index++)player.hand.push({id:'wound',uid:`wound-${state.turn}-${source}-${player.wounds}-${index}`,name:'Wound',color:'wound',basic:{},strong:{}});}
...
function resolvePvpAction(state,action,actor){
  ...
  if(action.type==='PVP_DAMAGE_UNIT'){if(role!=='attack'||pvp.stage!=='damage')return fail(state,'Only the attacker assigns remaining damage.');const target=playerById(state,opponentId),unit=findUnit(target.units,action.unitId);if(!unit||unit.wounded)return fail(state,'Choose an unwounded opposing Unit.');const resistances=unit.resistances||[],resistant=resistances.some(element=>pvp.attackElements.includes(element))||(pvp.attackElements.includes('coldfire')&&resistances.includes('fire')&&resistances.includes('ice')),cost=unit.armor*(resistant?2:1);if(pvp.damageRemaining<cost)return fail(state,`${cost} damage is required to wound that Unit.`);unit.wounded=true;unit.woundCount=1;pvp.damageRemaining-=cost;return state;}
...
function endTurn(state,roundAnnouncement=false,stage='start') {
  if(stage==='start'){
    const hex=applyForcedWithdrawal(state),timeBending=Boolean(state.bonuses.timeBending),immediateExtraTurn=Boolean(state.player.extraTurn)||timeBending,carry=state.player.carry,handLimitBonus=state.bonuses.handLimitBonus||0;state.player.extraTurn=false;state.player.carry=null;
    ...
    const played=state.player.played.splice(0);for(const card of played){const placement=card.endTurnPlacement;delete card.endTurnPlacement;if(timeBending){if(card.id==='space-bending')state.player.setAside.push(card);else state.player.hand.push(card);}else if(placement==='deck-top')state.player.deck.unshift(card);else if(placement==='deck-bottom')state.player.deck.push(card);else state.player.discard.push(card);}state.player.cardsPlayedThisTurn=0;state.player.atTurnStart=false;state.player.emptyHandPassAllowed=false;state.player.movedThisTurn=false;state.player.moveHistory=[];state.player.turnAction=null;state.player.cityInfluenceApplied=false;state.points=freshPoints();if(carry)Object.entries(carry).forEach(([key,value])=>state.points[key]+=value);state.mana=[];state.sourceTaken=false;resetTurnBonuses(state);state.player.skills.forEach(skill=>{skill.used=false;});
...
function advanceRound(state) {
  ...
  } else {resetTurnBonuses(state);state.player.deck=shuffled([...state.player.discard,...state.player.hand,...state.player.setAside],state.seed+state.round);state.player.hand=[];state.player.discard=[];state.player.setAside=[];state.player.timeBentTurn=false;state.player.tactic=null;state.player.tacticUsed=false;state.player.cardsPlayedThisTurn=0;state.player.atTurnStart=!state.tacticsEnabled;state.player.emptyHandPassAllowed=false;state.player.movedThisTurn=false;state.player.moveHistory=[];state.player.turnAction=null;state.player.cityInfluenceApplied=false;draw(state,handLimit(state)+(state.player.nextRoundHandBonus||0));state.player.nextRoundHandBonus=0;state.player.units.forEach(unit=>{unit.spent=false;if(unit.banner)unit.banner.used=false;});state.player.skills.forEach(s=>{s.used=false;s.roundUsed=false;});}
