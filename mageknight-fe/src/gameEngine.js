...
export function createGame(seed = 20260901, options = {}) {
  ...
  const state = {
    version:6, seed, scenario: 'Solo Conquest', status: 'playing', round: 1, maxRounds:6, time: 'day', turn: 1, phase: options.tactics ? 'tactic' : 'action', tacticsEnabled:Boolean(options.tactics), tactic:null,
...
export function createMultiplayerGame(lobby, seed = 20260901) {
  ...
  state.version=6;state.multiplayer=true;state.scenario=lobby.scenario;state.maxRounds=lobby.scenario==='blitz-conquest'?4:6;state.phase='tactic';state.tacticSelections={};state.turnOrder=[];state.activePlayerId=null;state.roundEndTurnsRemaining=null;state.tacticPickOrder=[...lobby.players].reverse().map(player=>player.id);state.tacticPickerId=state.tacticPickOrder[0];
...
function assignCombatDamage(state,enemy){
  ...
  for(const id of state.combat.damageUnits||[]){const unit=state.player.units.find(item=>item.id===id);if(!unit||unit.wounded||damage<=0)continue;const resistant=(unit.resistances||[]).includes(attackType);if(resistant){damage=Math.max(0,damage-unit.armor);if(damage<=0){state.combat.damageAssignedUnitIds=state.combat.damageAssignedUnitIds||[];state.combat.damageAssignedUnitIds.push(unit.id);continue;}}vampiricGain++;if(enemy.traits.includes('paralyze')){if(unit.banner)state.player.discard.push(unit.banner);state.player.units=state.player.units.filter(item=>item.id!==unit.id);log(state,`${unit.name} was destroyed by Paralyze.`);}else{unit.wounded=true;unit.woundCount=enemy.traits.includes('poison')?2:1;log(state,`${unit.name} received ${unit.woundCount} Wound${unit.woundCount===1?'':'s'}.`);}damage=Math.max(0,damage-unit.armor);}
