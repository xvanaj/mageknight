...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'RESOLVE_ATTACK': {
      ...
      const adjusted=targets.map(enemy=>enemy.traits.includes('elusive')&&!state.combat.blockedIds.includes(enemyKey(enemy))?{...enemy,armor:Math.ceil(enemy.armor*1.5)}:enemy),power={physical:state.points.attack+state.points.ranged+state.points.siege,ice:state.points.iceAttack,fire:state.points.fireAttack,coldfire:state.points.coldfireAttack},required=adjusted.reduce((sum,enemy)=>sum+enemy.armor,0);if(!canDefeatWithAttack(adjusted,power))return fail(state,`The effective Attack cannot defeat this group (${required} total Armor; ${power.physical} physical, ${power.ice} ice, ${power.fire} fire, ${power.coldfire} cold fire available).`);
      markCombatDefeated(state,targets);spendCombatPoints(state,['attack','iceAttack','fireAttack','coldfireAttack','ranged','siege']);log(state,`${targets.map(enemy=>enemy.name).join(', ')} defeated in melee.`);if(!livingCombatEnemies(state).length)return winCombat(state,'attack');return state;
    }
