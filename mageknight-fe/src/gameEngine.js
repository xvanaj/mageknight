...
export function createGame(seed = 20260901, options = {}) {
  ...
  const state = {
    ...
    map:prepareMap(Boolean(options.exploration)), tileDeck:[...shuffled(MAP_TILES.filter(tile=>!tile.core).map(tile=>tile.id),seed+23),...shuffled(MAP_TILES.filter(tile=>tile.core).map(tile=>tile.id),seed+29)], exploredTiles:[], enemyDecks:createEnemyDecks(seed),enemyDiscards:{brown:[]},explorationEnabled:Boolean(options.exploration), source,
    ...
    scoring:null,pvp:null,cooperativeAssault:null,scenarioEndTurnsRemaining:null,roundWasAnnounced:false,removedTactics:{day:[],night:[]},pendingTacticRemoval:null,
  };
