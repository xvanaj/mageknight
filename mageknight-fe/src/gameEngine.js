...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'START_COMBAT': {
      ...
      const defenders=clone(hex.enemy.members||hex.enemies||[hex.enemy]).map(enemy=>({...enemy,siteFortified:kind==='fortified'}));let enemies=defenders,enemySources=Object.fromEntries(defenders.map(enemy=>[enemyKey(enemy),{q:hex.q,r:hex.r,site:hex.site,siteDefender:true}]));
