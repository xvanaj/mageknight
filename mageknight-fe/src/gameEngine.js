...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'BLOCK_ENEMY': {
      ...
      const required=enemy.attack*((enemy.traits||[]).includes('swift')?2:1),power=effectiveBlock(state,enemy);spendCombatPoints(state,['block','iceBlock','fireBlock','coldfireBlock']);if(power<required)return fail(state,`That block failed: ${required} was required, but only ${power} was committed.`);state.combat.blockedIds.push(blockEnemyKey(enemy));log(state,`${enemy.name}'s attack was blocked separately.`);return state;
    }
    case 'RESOLVE_BLOCK': {
