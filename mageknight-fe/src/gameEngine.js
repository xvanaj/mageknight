...
export const UNITS = [
  {id:'peasants',name:'Peasants',copies:3,level:1,cost:4,armor:3,sites:['village'],ability:{attack:2,block:2,influence:2,move:2}},
  {id:'herbalists',name:'Herbalists',copies:2,level:1,cost:3,armor:2,sites:['village','monastery'],ability:{}},
  {id:'foresters',name:'Foresters',copies:2,level:1,cost:5,armor:4,sites:['village'],ability:{block:3,move:2}},
  {id:'utem-crossbowmen',name:'Utem Crossbowmen',copies:2,level:2,cost:6,armor:4,sites:['village','keep'],ability:{attack:3,block:3,ranged:2}},
  {id:'utem-guardsmen',name:'Utem Guardsmen',copies:2,level:2,cost:5,armor:5,sites:['village','keep'],ability:{attack:2,block:4}},
  {id:'utem-swordsmen',name:'Utem Swordsmen',copies:2,level:2,cost:6,armor:4,sites:['keep'],ability:{attack:3,block:3}},
  {id:'guardian-golems',name:'Guardian Golems',copies:2,level:2,cost:7,armor:3,sites:['mage-tower','keep'],ability:{attack:2,block:2}},
  {id:'illusionists',name:'Illusionists',copies:2,level:2,cost:7,armor:2,sites:['mage-tower','monastery'],ability:{influence:4,whiteCrystal:1}},
  {id:'red-cape-monks',name:'Red Cape Monks',copies:1,level:2,cost:7,armor:4,sites:['monastery'],ability:{attack:3,block:3}},
  {id:'northern-monks',name:'Northern Monks',copies:1,level:2,cost:7,armor:4,sites:['monastery'],ability:{attack:3,block:3}},
  {id:'savage-monks',name:'Savage Monks',copies:1,level:2,cost:7,armor:4,sites:['monastery'],ability:{attack:3,block:3}},
  {id:'fire-golems',name:'Fire Golems',copies:2,level:3,cost:8,armor:4,sites:['keep','mage-tower'],elite:true,ability:{attack:3,block:3}},
  {id:'ice-golems',name:'Ice Golems',copies:2,level:3,cost:8,armor:4,sites:['keep','mage-tower'],elite:true,ability:{iceAttack:3,iceBlock:3}},
  {id:'fire-mages',name:'Fire Mages',copies:2,level:3,cost:9,armor:3,sites:['mage-tower','monastery'],elite:true,ability:{ranged:3,redCrystal:1}},
  {id:'ice-mages',name:'Ice Mages',copies:2,level:3,cost:9,armor:4,sites:['mage-tower','monastery'],elite:true,ability:{iceAttack:4,iceBlock:4,blueCrystal:1}},
  {id:'amotep-gunners',name:'Amotep Gunners',copies:2,level:3,cost:8,armor:6,sites:['keep','city'],elite:true,ability:{attack:5,block:5}},
  {id:'amotep-freezers',name:'Amotep Freezers',copies:2,level:3,cost:8,armor:6,sites:['keep','city'],elite:true,ability:{attack:5,block:5}},
  {id:'catapults',name:'Catapults',copies:3,level:3,cost:9,armor:4,sites:['keep','city'],elite:true,ability:{siege:3}},
  {id:'altem-mages',name:'Altem Mages',copies:2,level:4,cost:12,armor:5,sites:['city'],elite:true,ability:{coldfireAttack:5,coldfireBlock:5}},
  {id:'altem-guardians',name:'Altem Guardians',copies:3,level:4,cost:11,armor:7,sites:['city'],elite:true,ability:{attack:5,block:8}},
];
...
const migrateState=state=>{
  ...
  state.decks=state.decks||{};state.offer.monastery=state.offer.monastery||[];if((state.version||0)<7)migrateUnitCards(state);state.enemyDecks=state.enemyDecks||createEnemyDecks(state.seed||1);state.enemyDiscards=state.enemyDiscards||{};state.enemyDiscards.brown=state.enemyDiscards.brown||[];state.scenarioEndTurnsRemaining=state.scenarioEndTurnsRemaining??null;state.roundWasAnnounced=Boolean(state.roundWasAnnounced);state.undoBlockedReason=state.undoBlockedReason||null;state.removedTactics=state.removedTactics||{day:[],night:[]};state.removedTactics.day=state.removedTactics.day||[];state.removedTactics.night=state.removedTactics.night||[];state.pendingTacticRemoval=state.pendingTacticRemoval||null;state.commonSkills=state.commonSkills||[];if(state.pvp?.prepared)Object.keys(state.pvp.prepared).forEach(id=>{state.pvp.prepared[id]={...freshPoints(),...state.pvp.prepared[id]};});if(state.combat){state.combat.damageUnits=state.combat.damageUnits||[];state.combat.damageAssignedUnitIds=state.combat.damageAssignedUnitIds||[];state.combat.enemies=state.combat.enemies||clone(state.combat.enemy?.members||[state.combat.enemy].filter(Boolean));state.combat.defeatedIds=state.combat.defeatedIds||[];state.combat.blockedIds=state.combat.blockedIds||[];}migrateRuinsState(state);state.version=7;return state;
};
...
export function createGame(seed = 20260901, options = {}) {
  ...
  const state = {
    version:7, seed, scenario: 'Solo Conquest', status: 'playing', round: 1, maxRounds:6, time: 'day', turn: 1, phase: options.tactics ? 'tactic' : 'action', tacticsEnabled:Boolean(options.tactics), tactic:null,
...
export function createMultiplayerGame(lobby, seed = 20260901) {
  ...
  state.version=7;state.multiplayer=true;state.scenario=lobby.scenario;state.maxRounds=lobby.scenario==='blitz-conquest'?4:6;state.phase='tactic';state.tacticSelections={};state.turnOrder=[];state.activePlayerId=null;state.roundEndTurnsRemaining=null;state.tacticPickOrder=[...lobby.players].reverse().map(player=>player.id);state.tacticPickerId=state.tacticPickOrder[0];
...
const canPayManaCost=(state,cost)=>{const gold=effectiveTime(state)==='day'?state.mana.filter(color=>color==='gold').length:0,shortfall=cost.reduce((sum,item)=>sum+Math.max(0,item.count-state.mana.filter(color=>color===item.color).length-(state.player.crystals[item.color]||0)),0);return shortfall<=gold;};
function payManaCost(state,cost){if(!canPayManaCost(state,cost))return false;let goldNeeded=0;for(const item of cost){let remaining=item.count;while(remaining&&state.mana.includes(item.color)){state.mana.splice(state.mana.indexOf(item.color),1);remaining--;}while(remaining&&(state.player.crystals[item.color]||0)>0){state.player.crystals[item.color]--;remaining--;}goldNeeded+=remaining;}while(goldNeeded--){const index=state.mana.indexOf('gold');if(index<0)return false;state.mana.splice(index,1);}return true;}
...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'INITIATE_PVP': {
      ...
      state.points.heal=0;state.player.turnAction='combat';state.pvp={attackerId:state.player.id,defenderId:defender.id,aggressorOrigin:{q:state.player.moveHistory[state.player.moveHistory.length-1]?.q??state.player.q,r:state.player.moveHistory[state.player.moveHistory.length-1]?.r??state.player.r},phase:'attend',attendance:null,currentAttackerId:null,stage:'attend',passes:0,attackPower:0,attackElements:[],blockPower:0,blockTypes:{physical:0,fire:0,ice:0,coldfire:0},damageRemaining:0,prepared:{[state.player.id]:{ranged:state.points.ranged,siege:state.points.siege,attack:state.points.attack,iceAttack:state.points.iceAttack,fireAttack:state.points.fireAttack,coldfireAttack:state.points.coldfireAttack,block:state.points.block,iceBlock:state.points.iceBlock,fireBlock:state.points.fireBlock,coldfireBlock:state.points.coldfireBlock},[defender.id]:freshPoints()}};spendCombatPoints(state,['ranged','siege','attack','iceAttack','fireAttack','coldfireAttack','block','iceBlock','fireBlock','coldfireBlock']);state.phase='pvp-attend';log(state,`${state.player.name} entered ${defender.name}'s space and initiated PvP.`);return state;
    }
    case 'START_COMBAT': {
    ...
    case 'RESOLVE_RANGED': {
      ...
      state.combat.attackStarted=true;markCombatDefeated(state,targets);spendCombatPoints(state,['ranged','siege']);log(state,`${targets.map(enemy=>enemy.name).join(', ')} defeated with Ranged/Siege Attack.`);if(!livingCombatEnemies(state).length)return winCombat(state,'ranged');return state;
    }
    ...
    case 'RESOLVE_BLOCK': {
      if(state.phase!=='combat-block')return fail(state,'Not in the Block phase.');const enemies=attackingCombatEnemies(state);if(enemies.length!==1)return fail(state,'Multiple enemy attacks must be blocked separately.');const enemy=enemies[0],required=enemy.attack*((enemy.traits||[]).includes('swift')?2:1),power=effectiveBlock(state,enemy);spendCombatPoints(state,['block','iceBlock','fireBlock','coldfireBlock']);state.combat.blocked=power>=required;if(state.combat.blocked){state.combat.blockedIds.push(blockEnemyKey(enemy));log(state,'Attack fully blocked.');}return finishBlockPhase(state);
    }
...
const pvpCardPower=(effect,phase,role,effectAs,siegeOnly=false)=>{const flexible=effectAs?{[effectAs]:Number(effect.any||effect.anyCombat||0)}:{};const value={...effect,...flexible};if(role==='block'){const blocks={physical:Number(value.block||0),fire:Number(value.fireBlock||0),ice:Number(value.iceBlock||0),coldfire:Number(value.coldfireBlock||0)};return {power:Object.values(blocks).reduce((sum,amount)=>sum+amount,0),elements:[],blocks};}if(phase==='ranged')return {power:Number(value.siege||0)+(siegeOnly?0:Number(value.ranged||0)),elements:['physical']};const elements=[];if(value.attack||value.ranged||value.siege)elements.push('physical');if(value.fireAttack)elements.push('fire');if(value.iceAttack)elements.push('ice');if(value.coldfireAttack)elements.push('coldfire');return {power:Number(value.attack||0)+Number(value.ranged||0)+Number(value.siege||0)+Number(value.fireAttack||0)+Number(value.iceAttack||0)+Number(value.coldfireAttack||0),elements};};
...
function resolvePvpAction(state,action,actor){
  ...
  if(action.type==='PVP_USE_UNIT'){
    if(pvp.stage!==role)return fail(state,`It is not your turn to ${role}.`);const unit=findUnit(actor.units,action.id);if(!unit||unit.spent||unit.wounded)return fail(state,'That Unit is unavailable or wounded.');const abilities=legalPvpEffectTypes(unit.ability,pvp.phase,role,siegeOnly);if(!abilities.length)return fail(state,`That Unit provides no usable ${role} power in this phase.`);if(abilities.length>1&&!abilities.includes(action.abilityAs))return fail(state,'Choose which Unit ability to use in PvP.');const ability=abilities.length===1?abilities[0]:action.abilityAs,part=pvpCardPower({[ability]:unit.ability[ability]},pvp.phase,role,null,siegeOnly);if(role==='attack'){pvp.attackPower+=part.power;pvp.attackElements=[...new Set([...pvp.attackElements,...part.elements])];}else{pvp.blockPower+=part.power;Object.entries(part.blocks).forEach(([element,amount])=>{pvp.blockTypes[element]+=amount;});}unit.spent=true;log(state,`${actor.name} activated ${unit.name} for PvP ${role}.`);return state;
  }
...
function finishRangedPhase(state){
  spendCombatPoints(state,['ranged','siege']);state.combat.blockEnemies=livingCombatEnemies(state).flatMap(enemy=>{
...
function assignCombatDamage(state,enemy){
  ...
  for(const id of state.combat.damageUnits||[]){const unit=findUnit(state.player.units,id);if(!unit||unit.wounded||damage<=0)continue;const key=unitKey(unit),resistant=(unit.resistances||[]).includes(attackType);if(resistant){damage=Math.max(0,damage-unit.armor);if(damage<=0){state.combat.damageAssignedUnitIds=state.combat.damageAssignedUnitIds||[];state.combat.damageAssignedUnitIds.push(key);continue;}}vampiricGain++;if(enemy.traits.includes('paralyze')){if(unit.banner)state.player.discard.push(unit.banner);state.player.units=state.player.units.filter(item=>unitKey(item)!==key);log(state,`${unit.name} was destroyed by Paralyze.`);}else{unit.wounded=true;unit.woundCount=enemy.traits.includes('poison')?2:1;log(state,`${unit.name} received ${unit.woundCount} Wound${unit.woundCount===1?'':'s'}.`);}damage=Math.max(0,damage-unit.armor);}
