...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'ASSIGN_DAMAGE_UNIT': {
      if(state.phase!=='combat-damage'||!state.combat)return fail(state,'Choose Units while assigning one enemy attack.');if(unitsForbiddenInCombat(state))return fail(state,'Units cannot receive damage in a Dungeon, Tomb, or monastery-burning combat.');if((state.combat.damageQueue?.[0]?.traits||[]).includes('assassination'))return fail(state,'Assassination damage must be assigned to the Hero.');const unit=findUnit(state.player.units,action.id),key=unitKey(unit);if(!unit||unit.wounded||(state.combat.damageAssignedUnitIds||[]).includes(key))return fail(state,'Only an unwounded unit not already assigned in this combat can receive damage.');state.combat.damageUnits=state.combat.damageUnits||[];const index=state.combat.damageUnits.indexOf(key);if(index>=0)state.combat.damageUnits.splice(index,1);else state.combat.damageUnits.push(key);return state;
    }
