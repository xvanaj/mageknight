...
function winCombat(state, phase) {
  ...
  if(!siteAlreadyConquered&&hex.site==='ruins'){const reward=hex.ruinsToken?.reward||'crystals';state.pendingRewards.push({type:reward,...(reward==='crystals'?{count:hex.ruinsToken?.count||4}:{}),source:'Ancient Ruins'});}
...
function interact(state, action) {
  ...
  if(action.kind==='altar'){if(hex.site!=='ruins'||hex.ruinsToken?.type!=='altar'||hex.ruinsToken.faceDown||hex.conquered||hex.used)return fail(state,'There is no revealed unconquered altar here.');const color=hex.ruinsToken.color,available=state.mana.filter(item=>item===color).length+(state.time==='day'?state.mana.filter(item=>item==='gold').length:0)+(state.player.crystals[color]||0);if(available<3)return fail(state,`The altar requires 3 ${color} mana.`);for(let count=0;count<3;count++)spendMana(state,color);gainFame(state,hex.ruinsToken.fame);hex.used=true;hex.conquered=true;hex.ownerId=state.player.id||state.player.character;log(state,`Three ${color} mana invoked the altar for ${hex.ruinsToken.fame} Fame.`);return state;}
