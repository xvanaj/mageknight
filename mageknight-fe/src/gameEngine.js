...
function addEffect(state, effect, sidewaysAs, choices={}) {
  if (sidewaysAs) {
    ...
    if (resolved in state.points) state.points[resolved] += amount;
      ...
      if(amount.mayCrystallize&&choices.crystalColor){const index=state.mana.lastIndexOf(choices.crystalColor);if(index>=0){state.mana.splice(index,1);gainCrystal(state,choices.crystalColor);}}
    }
...
function spendMana(state, color) {
  ...
  if ((effectiveTime(state) === 'day'||state.bonuses?.goldAsBasic) && i >= 0) { state.mana.splice(i, 1); return true; }
  i = state.mana.indexOf('black');
  if (state.bonuses?.blackAsBasic && i >= 0) { state.mana.splice(i, 1); return true; }
  if (state.player.crystals[color] > 0) { state.player.crystals[color]--;state.player.crystalsSpentThisTurn[color]=(state.player.crystalsSpentThisTurn[color]||0)+1;return true; }
  return false;
}
...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'PLAY_CARD': {
      ...
      if(effect.pureMagic){state.mana.splice(state.mana.indexOf(action.pureColor),1);if(action.mode==='strong'&&!pendingBoost)payManaCost(state,[{color:'blue',count:1}]);}
...
function endTurn(state,roundAnnouncement=false,stage='start') {
  ...
  if(state.multiplayer){
    ...
    if(immediateExtraTurn){state.player.timeBentTurn=Boolean(context.timeBending);state.player.atTurnStart=true;state.player.emptyHandPassAllowed=!state.player.hand.length&&Boolean(state.player.deck.length);log(state,context.timeBending?`${state.player.name} bends time and immediately takes another turn.`:`${state.player.name} takes the extra turn granted by ${state.player.tactic?.name}.`);checkpointTurn(state);return state;}
