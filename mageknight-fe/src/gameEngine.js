...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'START_COOPERATIVE_ASSAULT': {
      ...
      state.player.reputation=Math.max(-7,state.player.reputation-1);state.player.turnAction='combat';state.cooperativeAssault={q:hex.q,r:hex.r,leaderId:state.player.id,eligible,contributors:{[state.player.id]:{attack:state.points.attack+state.points.iceAttack+state.points.fireAttack,ranged:state.points.ranged,siege:state.points.siege,block:state.points.block+state.points.iceBlock+state.points.fireBlock}},ready:[state.player.id]};state.phase='team-assault';log(state,`Joint assault on the ${hex.cityColor} city begins.`);return state;
    }
...
function contributeRemote(state,action){
  ...
  if(action.type==='TEAM_CONTRIBUTE'){
    ...
    if(action.ready){if(!assault.ready.includes(actor.id)){assault.ready.push(actor.id);if(actor.id!==assault.leaderId){actor.skipNextTurn=true;actor.roundOrderFaceDown=true;}actor.reputation=Math.max(-7,actor.reputation-1);}bindPlayerState(state,state.activePlayerId);return state;}
