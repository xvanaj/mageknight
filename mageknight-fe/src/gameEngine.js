...
const migrateState=state=>{
  ...
  state.bonuses={...freshBonuses(),...(state.bonuses||{})};state.pendingCardBoost=state.pendingCardBoost||null;state.pendingBorrowedEffect=state.pendingBorrowedEffect||null;state.pendingRepeatedEffect=state.pendingRepeatedEffect||null;state.pendingSpellEffect=state.pendingSpellEffect||null;
...
const clearCombatBonuses=state=>{state.bonuses.unitCombatBonus=null;state.bonuses.unitsCannotAbsorbDamage=false;};
...
export function createGame(seed = 20260901, options = {}) {
  ...
  const state = {
    ...
    points: freshPoints(), mana: [], sourceTaken: false, combat: null, pendingRewards:[], bonuses:freshBonuses(),pendingCardBoost:null,pendingBorrowedEffect:null,pendingRepeatedEffect:null,pendingSpellEffect:null, log: [], error: null,
...
const EFFECT_PHASES = {
  ...
  influence:['action'], heal:['action'], draw:['action'], mana:['action'], command:['action'], fame:['action'], unitReady:['action'], restoration:['action'],
...
export function reduceGame(input, action) {
  ...
  switch (action.type) {
    ...
    case 'RESOLVE_BLOCK': {
      if(state.phase!=='combat-block')return fail(state,'Not in the Block phase.');const enemies=attackingCombatEnemies(state);if(enemies.length!==1)return fail(state,'Multiple enemy attacks must be blocked separately.');const enemy=enemies[0],required=enemy.attack*((enemy.traits||[]).includes('swift')?2:1),power=effectiveBlock(state,enemy);spendCombatPoints(state,['block','iceBlock','fireBlock','coldfireBlock']);state.bonuses.swiftBlock=0;state.combat.blocked=power>=required;if(state.combat.blocked){state.combat.blockedIds.push(blockEnemyKey(enemy));log(state,'Attack fully blocked.');}return finishBlockPhase(state);
    }
