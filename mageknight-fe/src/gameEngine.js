...
const rangedRequirements=enemies=>{const fortifications=enemies.map(enemy=>Number(Boolean(enemy.siteFortified))+Number((enemy.traits||[]).includes('fortified'))),impossible=fortifications.some(value=>value>1),siegeOnly=fortifications.some(Boolean),resistant=enemies.some(enemy=>(enemy.traits||[]).includes('physical-resistant')),required=enemies.reduce((sum,enemy)=>sum+enemy.armor,0)*(resistant?2:1);return {siege:siegeOnly?required:0,open:siegeOnly?0:required,impossible};};
