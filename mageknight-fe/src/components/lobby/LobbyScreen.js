  ...
  return <div className="lobby-shell">
    ...
    <main className="lobby-layout">
      ...
      <aside className="lobby-sidebar">
        ...
        {route.isHost&&<button className="start-button" disabled={!canStartLobby(lobby)} onClick={()=>sendAction({type:'START'})}>Start game<span>{lobby?.players.length<2?'Waiting for another player':canStartLobby(lobby)?'Begin the conquest':'Waiting for everyone to be ready'}</span></button>}
      </aside>
    </main>
  </div>;
}
...
function NewGamePanel({name,setName,onCreate}) {
  ...
  return <div className="entry-shell"><LobbyHeader/><main className="entry-card"><span className="entry-rune">MK</span><span className="eyebrow">Online conquest</span><h1>Create a new game</h1><p>Host a private lobby, invite up to three other players, choose characters, and begin when everyone is ready.</p><form onSubmit={submit}><label>Your name<input autoFocus maxLength="24" value={name} onChange={event=>setName(event.target.value)} placeholder="Enter your name"/></label><button type="submit" disabled={!name.trim()}>Create game<span>Generate an invite link</span></button></form></main></div>;
}
