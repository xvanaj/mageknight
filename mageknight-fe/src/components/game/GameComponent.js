...
function GameComponent({session,onLeaveGame,multiplayerGame,onGameAction,networkStatus}) {
  ...
  const interactiveRemote=['team-assault','pvp-attend','pvp-ranged','pvp-melee'].includes(game.phase);
  ...
  return <div className={`game-shell ${waitingForTurn?'not-my-turn':''}`}>
    ...
    <section className="hand-panel"><div className="hand-heading"><div><span className="eyebrow">Your hand</span><h2>{game.player.hand.length} cards</h2></div><span>Click an action to commit a card. Strong actions consume matching mana.</span></div><div className="hand-scroll">{game.player.hand.map(card=><DeedCard card={card} hand={game.player.hand} units={game.player.units} phase={game.phase} key={card.uid} onPlay={play} onDiscard={discard} sideways={sideways} setSideways={setSideways}/>)}</div></section>
...
function phaseName(p){return ({tactic:'Choose tactic',action:'Action phase','combat-ranged':'Ranged / Siege','combat-block':'Block','combat-damage':'Assign damage','combat-attack':'Attack','team-assault':'Joint city assault','pvp-attend':'PvP reaction','pvp-ranged':'PvP ranged','pvp-melee':'PvP melee'})[p]||p}
...
function DeedCard({card,hand,units,phase,onPlay,onDiscard,sideways,setSideways}){
  ...
  return <article className={`deed ${card.color}`}><div className="card-band"><span>{card.type||card.color}</span><i/></div><h3>{card.name}</h3>{choicesFor('basic',basicLabel)}{choicesFor('strong',strongLabel,'strong')}{pending&&<div className="card-choice discard-choice"><span>Discard another card</span>{hand.filter(item=>item.uid!==card.uid&&item.id!=='wound').map(item=><button key={item.uid} onClick={()=>{onPlay(card.uid,pending.mode,null,{...pending.choices,discardUid:item.uid});setPending(null)}}>{item.name}</button>)}<button onClick={()=>setPending(null)}>Cancel</button></div>}<button className="sideways" onClick={()=>setSideways(sideways===card.uid?null:card.uid)}>Play sideways · +1</button>{sideways===card.uid&&<div className="sideways-menu">{['move','influence','attack','block'].map(as=><button onClick={()=>onPlay(card.uid,'sideways',as)} key={as}>{as}</button>)}</div>}<button className="sideways" onClick={()=>onDiscard(card.uid)}>Discard</button></article>;
}
...
function SkillControl({skill,game,dispatch}){
  ...
  if(['any','anyCombat'].includes(skill.effect)){const options=skill.effect==='any'?['move','influence','attack','block']:game.phase==='combat-ranged'?['ranged','siege']:game.phase==='combat-block'?['block']:['attack'];return <div className="skill-chip"><b>{skill.name}</b><span>{skill.description}</span><div>{options.map(mode=><button disabled={disabled} key={mode} onClick={()=>dispatch({type:'USE_SKILL',id:skill.id,mode})}>{mode}</button>)}</div></div>;}
...
function Combat({game,dispatch}){
  ...
  return <div className="combat-card"><div className="enemy-title"><span>⚔</span><div><h3>{e.name}</h3><p>Select one enemy or a group for each attack. Grouped enemies share their resistances.</p></div></div>
    ...
    {game.phase==='combat-damage'&&<><p><b>{combat.damageQueue[0].name}</b> deals {combat.damageQueue[0].attack}{combat.damageQueue[0].traits.includes('brutal')?' × 2 Brutal':''} damage. Select any unwounded Units for this attack, in the order clicked; the rest goes to your Hero.</p>{game.player.units.filter(unit=>!unit.wounded).map(unit=><button key={unit.id} onClick={()=>dispatch({type:'ASSIGN_DAMAGE_UNIT',id:unit.id})}>{assigned.has(unit.id)?'✓ ':''}Assign this attack to {unit.name}<span>Armor {unit.armor}{unit.resistances?.length?` · resists ${unit.resistances.join(', ')}`:''}</span></button>)}<button className="primary wide" onClick={()=>dispatch({type:'RESOLVE_DAMAGE'})}>Assign this enemy's damage<span>{combat.damageQueue.length-1} attacks remain after this</span></button></>}
...
function PvpPanel({game,dispatch}){const duel=game.pvp,mine=game.viewerPlayerId||game.player.id,isDefender=duel.defenderId===mine,isAttacker=duel.attackerId===mine,attacker=game.players.find(player=>player.id===duel.attackerId),retreats=isDefender?game.map.filter(hex=>hex.revealed!==false&&hexDistance(game.player,hex)===1&&hexDistance(attacker,hex)>1&&!['lake','mountain'].includes(hex.terrain)&&!game.players.some(player=>player.id!==mine&&player.q===hex.q&&player.r===hex.r)):[];
