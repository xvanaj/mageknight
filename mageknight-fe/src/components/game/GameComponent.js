...
function Combat({game,dispatch}){
  const combat=game.combat,e=combat.enemy,enemies=combat.enemies||e.members||[e],defeated=new Set(combat.defeatedIds||[]),blocked=new Set(combat.blockedIds||[]),alive=enemies.filter(enemy=>!defeated.has(enemy.uid||enemy.id)),assigned=new Set(combat.damageUnits||[]);
  ...
  return <div className="combat-card"><div className="enemy-title"><span>⚔</span><div><h3>{e.name}</h3><p>Select one enemy or a group for each attack. Grouped enemies share their resistances.</p></div></div>
    {enemies.map(enemy=>{const id=enemy.uid||enemy.id,isDefeated=defeated.has(id),isBlocked=blocked.has(id);return <button className={`enemy-choice ${validSelected.includes(id)?'selected':''}`} disabled={isDefeated||(game.phase==='combat-block'&&isBlocked)} onClick={()=>game.phase==='combat-block'?dispatch({type:'BLOCK_ENEMY',targetId:id}):toggle(id)} key={id}><b>{isDefeated?'✓ ':isBlocked?'◆ ':''}{enemy.name}</b><span>Armor {enemy.armor} · Attack {enemy.attack} · Fame {enemy.fame} · {(enemy.traits||[]).join(', ')||'no abilities'}</span></button>})}
