...
function Combat({game,dispatch}){
  ...
  return <div className="combat-card"><div className="enemy-title"><span>⚔</span><div><h3>{e.name}</h3><p>Select one enemy or a group for each attack. Grouped enemies share their resistances.</p></div></div>
    ...
    {game.phase==='combat-block'&&<><p>Generate Block and click exactly one enemy. Every click commits the current Block only to that attack; excess is lost.</p>{game.player.units.filter(unit=>!unit.wounded).map(unit=><button key={unit.id} onClick={()=>dispatch({type:'ASSIGN_DAMAGE_UNIT',id:unit.id})}>{assigned.has(unit.id)?'✓ ':''}Assign upcoming damage to {unit.name}<span>Armor {unit.armor}</span></button>)}<button className="primary wide" onClick={()=>dispatch({type:'FINISH_BLOCK'})}>Finish blocks and assign damage</button></>}
