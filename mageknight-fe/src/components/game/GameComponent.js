...
function DeedCard({card,onPlay,sideways,setSideways}){
  ...
  return <article className={`deed ${card.color}`}><div className="card-band"><span>{card.type||card.color}</span><i/></div><h3>{card.name}</h3><button onClick={()=>onPlay(card.uid,'basic')}><span>{basicLabel}</span><b>{effectText(card.basic)}</b></button><button className="strong" onClick={()=>onPlay(card.uid,'strong')}><span>{strongLabel}</span><b>{effectText(card.strong)}</b></button><button className="sideways" onClick={()=>setSideways(sideways===card.uid?null:card.uid)}>Play sideways · +1</button>{sideways===card.uid&&<div className="sideways-menu">{['move','influence','attack','block'].map(as=><button onClick={()=>onPlay(card.uid,'sideways',as)} key={as}>{as}</button>)}</div>}</article>
}
